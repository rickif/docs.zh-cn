...
|empty_load_as_error|TRUE|导入数据为空时，是否返回报错提示 `all partitions have no load data`。<br>取值为TRUE，当导入数据为空时，导入失败，返回报错提示 `all partitions have no load data`。<br>取值为FALSE，当导入数据为空时，导入成功，返回 `OK`，不返回报错提示。|
